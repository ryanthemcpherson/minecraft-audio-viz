...
export class PreviewManager {
    ...
    async initPreview() {
        ...
        if (typeof THREE === 'undefined') {
            ...
            if (!loaded || typeof THREE === 'undefined') {
                this._failed = true;
                this.app._showToast('Three.js failed to load; 3D Preview disabled', 'warning');
                return;
            }
        }
        ...
        try {
            ...
            this.app._showToast('3D Preview initialization failed; disabled for this session', 'warning');
        }
    }
    ...
    scanStageBlocks() {
        if (!this.state.selectedStage) {
            this.app._showToast('No stage selected', 'warning');
            return;
        }
        if (!this.ws || !this.ws.isConnected) {
            this.app._showToast('Not connected', 'error');
            return;
        }
        this.ws.send({
        ...
        this.app._showToast('Scanning stage blocks...', 'info');
    }

    handleStageBlocks(data) {
        if (data.error) {
            this.app._showToast(data.error, 'error');
            return;
        }
        this._stageBlockData = data;
        this._renderStageBlocks(data);
        this.app._showToast(`Scanned ${data.blocks.length} blocks`, 'success');
    }
    ...
    _setupControls() {
        ...
        if (parityBtn) {
            parityBtn.addEventListener('click', () => this.app._requestParityCheck());
        }
    ...
    _animate() {
        ...
        try {
            ...
            this._failed = true;
            this.stopAnimation();
            this.app._showToast('3D Preview render error; disabled for this session', 'warning');
        }
    }
    ...
    updateFromAudioState() {
        ...
        try {
            ...
            this._failed = true;
            this.stopAnimation();
            this.app._showToast('3D Preview update failed; disabled for this session', 'warning');
        }
    }
    ...
    _updateMultiZone(zoneEntities, bands) {
        ...
        for (const [zoneName, entities] of Object.entries(zoneEntities)) {
            ...
            if (this.app._getZoneRenderMode(zoneName) === 'bitmap') {
