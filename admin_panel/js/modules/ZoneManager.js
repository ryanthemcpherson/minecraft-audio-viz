...
export class ZoneManager {
    ...
    syncBitmapStateFromZonePatterns() {
        ...
        for (const [zoneName, info] of Object.entries(zp)) {
            ...
            if (rm === 'bitmap') {
                ...
                if (this.app.preview?._bitmapPreview) {
                    const zg = this.app.preview._previewZoneGroups?.[zoneName];
                    if (zg) {
                        const bw = this.state.bitmap.width || 16;
                        const bh = this.state.bitmap.height || 12;
                        const patId = typeof info === 'object' ? info.pattern : info;
                        this.app.preview._bitmapPreview.activate(zoneName, bw, bh, patId || 'bmp_spectrum', zg);
                        this.app.preview._bitmapPreview.setZoneVisible(zoneName, true);
                    }
                }
            } else {
                this.state.bitmap.initializedZones.delete(zoneName);
                if (this.app.preview?._bitmapPreview) {
                    this.app.preview._bitmapPreview.deactivate(zoneName);
                }
            }
        }
    }
    ...
    handleZonesList(data) {
        ...
        this.renderZoneChips();
        this.app.bitmap.updateBitmapZoneSelector();

        if (this.app.preview?._initialized) {
