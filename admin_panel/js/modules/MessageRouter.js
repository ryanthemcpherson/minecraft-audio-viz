        ...
        switch (data.type) {
            case 'vj_state':
                ...
                if (data.zone_patterns) {
                    ...
                    app.bitmap.updateBitmapZoneSelector();
                }
                if (data.stages) {
                ...
                if (data.bitmap_zones) {
                    for (const [zoneName, info] of Object.entries(data.bitmap_zones)) {
                        if (info.initialized) {
                            ...
                            app.bitmap.updateBitmapStatus(info);
                        }
                    }
                }
            ...
            case 'bitmap_initialized': {
                ...
                app.bitmap.updateBitmapStatus(data);
                app.ui.showToast(`Bitmap initialized: ${data.width || '?'}x${data.height || '?'}`, 'success');
                // Show bitmap in 3D preview
                if (app.preview?._bitmapPreview) {
                    const zg = app.preview._previewZoneGroups[initZone];
                    if (zg) {
                        app.preview._bitmapPreview.activate(initZone, data.width || 16, data.height || 12,
                            data.pattern || app.state.bitmap.activePattern || 'bmp_plasma', zg);
                    }
                    app.preview._bitmapPreview.setZoneVisible(initZone, true);
                }
                break;
            }
            ...
            case 'bitmap_transition_started':
                ...
                app.bitmap.highlightBitmapPattern(data.pattern);
                if (app.preview?._bitmapPreview && data.zone) {
                    app.preview._bitmapPreview.setPattern(data.zone, data.pattern);
                }
                break;

            case 'bitmap_palette_set':
                app.state.bitmap.activePalette = data.palette;
                app.bitmap.highlightBitmapPalette(data.palette);
                break;
