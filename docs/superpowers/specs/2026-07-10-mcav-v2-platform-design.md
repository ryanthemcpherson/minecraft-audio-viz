...
- An optional Fabric client mod provides enhanced GPU-driven rendering.
...
- enhanced-client discovery and short-lived grant issuance.

It does not contain pattern implementations, DSP, beat detection, authoring logic, or a second scene runtime.

#### Enhanced Fabric client

The optional enhanced client renders high-fidelity plans using GPU instancing, high-density particles, constrained material graphs, post-processing, volumetric beams, and camera-local effects.

Paper verifies player membership and issues a one-use grant bound to the player UUID, server identity, show epoch, and expiry. When reachable, the enhanced client connects directly to the Show Engine for the low-latency data path. A Paper relay is a compatibility fallback, not the latency target.

#### WebGPU preview

The preview consumes the same versioned ShowIR and renderer capability profiles. It supports:

- enhanced-profile rehearsal;
- conservative vanilla-profile simulation;
...
3. **Renderer plans** are capability-specific projections compiled from ShowIR for Paper, enhanced clients, and WebGPU preview.
...
- optional enhanced layers that retain the same timing, parameters, palette, and cue semantics;
- a declared cost model;
- explicit capability requirements.

The design does not force effects to the lowest common denominator. Enhanced renderers may substantially enrich an effect, but the enriched effect remains part of the same graph and show timeline.
...
- 50,000 enhanced-client GPU instances;
...
- enhanced rendering at 60 FPS on the reference GPU;
- bounded network and memory use with no unbounded queue.

### 7.2 Latency objectives

- Vanilla LAN onset-to-visible latency: at most 75 ms p95.
- Enhanced-client LAN onset-to-photon latency: at most 45 ms p95.
...
Vanilla-visible latency is measured using an instrumented reference client running the vanilla render path. Enhanced clients report sampled present timestamps directly.

### 7.3 Per-hop traces

Frames carry comparable monotonic timestamps for:

`capture_read → fast_feature_ready → engine_ingress → graph_applied → plan_ready → adapter_send → enhanced_present`
...
- vanilla and enhanced preview profiles;
...
5. resource-pack and enhanced-asset compilation;
...
- Enhanced-client grants are bound to player UUID, server identity, show epoch, capability request, and expiry, and are revoked when membership ends.
...
- Studio, Show Engine, Paper adapter, enhanced client, and show packs use separate signed release manifests.
...
- bounded visual golden captures for vanilla and enhanced reference scenes
...
- forced capture, engine, Paper, network, pack, and enhanced-client failures;
...
4. **Compatibility:** declared Paper/Minecraft, vanilla resource-pack, enhanced hardware, installer, firewall, and upgrade matrix passes.
...
`one audio source → fast/detail features → one AuthorGraph → Show Engine → one Paper stage → WebGPU preview`
...
- capability compiler and dual-profile preview;
- show-pack signing, registry contract, cache, and admin mode;
- atomic live graph replacement and diagnostics.

### Phase 5: Enhanced client

Deliver:

- Paper-mediated capability and membership handshake;
- one-use direct Show Engine grant;
- GPU-instanced primitives, particles, constrained materials, and post-processing;
- enhanced latency gate;
- vanilla fallback conformance for every shipped node;
- hardware-tier quality policies and compatibility matrix.
