...
4. Restart the server once, then retrieve the generated admin username, admin password, DJ credentials, and TLS certificate fingerprint from `/mcav-vj/FIRST_LOGIN.txt`.

No Python installation, package-manager command, node-level Docker change, manual shared-secret synchronization, manual certificate creation, or manual plugin configuration edit is required.
...
mcav-vj/
  ...
  state/
    ...
    tls.crt                       generated on first start
    tls.key                       generated on first start
...
The VJ CLI gains an explicit project-root option, supplied as `/home/container/mcav-vj`, rather than deriving assets from the executable's temporary or bundled module path. The modern CLI also gains explicit HTTP-port, TLS-certificate, and TLS-key options so deployment ports and transport security are not hidden constants.
...
3. Generates a unique admin username plus independent URL-safe first-login passwords for the administrator and DJ.
4. Stores only bcrypt password hashes in `state/dj_auth.json`.
5. Generates an ECDSA P-256 self-signed TLS certificate and private key for the admin HTTP and browser WebSocket listeners.
6. Stores the shared secret in `state/runtime.env` and the TLS private key in `state/tls.key` with owner-only permissions.
7. Writes the generated human-readable credentials and SHA-256 TLS certificate fingerprint to `FIRST_LOGIN.txt` with owner-only permissions.
8. Locates existing plugin JARs by reading `plugin.yml` or `paper-plugin.yml` from each JAR and matching the plugin name `AudioViz`.
9. Moves differing AudioViz JARs into a timestamped backup directory and atomically installs `release/AudioViz.jar` as `/plugins/AudioViz.jar`.
10. Preserves an existing `/plugins/AudioViz/config.yml`, creates a timestamped backup before modification, and updates only the loopback WebSocket address, port `8765`, and root `ws-secret` value.
11. Uses `release/plugin-config.default.yml` when the plugin configuration does not yet exist.
...
Existing valid `runtime.env`, authentication configuration, TLS identity, and first-login credentials are retained. Credentials, the shared secret, and the certificate are never regenerated merely because the server restarts or the deployment ZIP is refreshed. A certificate approaching expiry may be renewed only after its replacement and new fingerprint are written atomically.
...
Minecraft port:    8765
Admin HTTPS host:  0.0.0.0
Admin HTTPS port:  8080
Browser WebSocket: 8766 using WSS
DJ WebSocket:      9000
Metrics:           127.0.0.1:9001
Authentication:    /home/container/mcav-vj/state/dj_auth.json
```

The wrapper loads the shared secret from `state/runtime.env`, starts VJ with authentication and browser TLS enabled, verifies that the VJ process remains alive through its initial bind phase, and prints concise portal, preview, and DJ endpoints. It then executes the exact command following `--` with `exec`, leaving Paper attached to the Pterodactyl console for commands, signals, resource monitoring, and crash detection.
...
## Admin Authentication and Browser Transport

The unauthenticated web surface is limited to the login shell and its static assets. The server sends no live state, configuration, preview data, or control capability until authentication succeeds. Both `/` and `/preview/` present the same authentication gate before connecting to live data.

The login form requires the generated administrator username and password. Browser authentication messages contain both fields, and the server verifies the named VJ operator rather than accepting a password belonging to any operator. The browser must not accept credentials through URL parameters, write them to `localStorage` or `sessionStorage`, or include them in logs. Credentials remain only in memory for the active page and are cleared on logout or page close.

The browser WebSocket requires authentication as its first message within five seconds. Before successful authentication it accepts no other message and emits no state. Failed browser logins are limited to five attempts per source IP in a rolling 60-second window. Error responses do not reveal whether the username or password was incorrect.

The admin HTTP listener uses HTTPS and the browser listener uses WSS by default. Bootstrap generates a self-signed certificate because an SFTP-only administrator cannot provision a publicly trusted identity automatically. `FIRST_LOGIN.txt` includes its SHA-256 fingerprint for out-of-band verification. The browser will show a one-time trust warning until the administrator installs a trusted certificate.

Administrators may replace `state/tls.crt` and `state/tls.key` with a trusted certificate and key while preserving the documented paths and permissions. VJ validates the certificate/key pair before binding; an invalid pair leaves the prior valid pair untouched when changed through bootstrap.

The separate DJ transport retains its existing authentication and transport configuration in this delivery. A public DJ allocation must use the DJ client's supported WSS configuration with a trusted certificate or be restricted to a trusted network/VPN. The self-signed browser certificate is not silently trusted by the native DJ client.
...
The wrapper never enables `--no-auth`. The generated DJ and administrator credentials are distinct. The Minecraft shared secret and TLS private key are not printed to the console or written to `FIRST_LOGIN.txt`.

The generated certificate encrypts browser credentials but is self-signed and therefore does not establish public identity until its fingerprint is verified. Production administrators should install a trusted certificate or keep the allocations behind a trusted network/VPN. Automated public certificate issuance and external TLS proxy provisioning are outside this SFTP-only deployment package.
...
- Missing or invalid TLS state: preserve recoverable files, report the certificate path, then launch Paper without the internet-facing panel.
...
- Exact username/password matching without cross-operator password acceptance.
- Browser login rate limiting, generic failure responses, and five-second authentication timeout.
- Rejection of all state and control messages before browser authentication.
...
- Idempotent TLS identity generation, certificate fingerprint reporting, and certificate/key validation.
...
- TLS bootstrap or bind failure still launches the Paper sentinel command.
...
3. Confirm the portable VJ serves the admin login and preview login over HTTPS.
4. Confirm unauthenticated browser clients receive no state and authenticated clients connect over WSS.
5. Confirm ports `8080`, `8766`, and `9000` bind and metrics remain loopback-only on `9001`.
6. Start a Paper test server with the included plugin.
7. Confirm VJ reconnects to `127.0.0.1:8765` using the generated shared secret.
8. Restart the container and confirm credentials, TLS identity, configuration, and installed JAR remain unchanged.
9. Inject bootstrap and VJ failures and confirm the Paper sentinel still starts.
...
- Provisioning a publicly trusted certificate, TLS reverse proxy, or VPN.
