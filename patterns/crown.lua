...
function calculate(audio, config, dt)
    ...
    local entity_count = config.entity_count or 96
    ...
    -- === 5 SPIKES (40 entities, 8 per spike) ===
    local points_per_spike = 8
    for k = 0, 4 do
        ...
        for j = 0, points_per_spike - 1 do
            local t = j / (points_per_spike - 1)
    ...
    -- === JEWELS (5 entities, one at each spike tip) ===
    ...
    -- === BAND DETAIL (16 entities: 8 inner ring + 8 cross-braces) ===
    ...
    -- Cross-braces connecting inner to outer ring (8 points)
    for i = 0, 7 do
    ...
    -- === FILIGREE (11 entities: arches between adjacent spikes) ===
    -- 5 arches, ~2 points each = 10, plus 1 extra
    ...
    for k = 0, 4 do
        ...
        -- 2 points per arch (at 1/3 and 2/3 between spike bases)
        local arch_points = 2
        if k == 4 then arch_points = 3 end  -- extra point on last arch to reach 11
