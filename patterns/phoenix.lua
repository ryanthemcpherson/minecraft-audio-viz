...
function calculate(audio, config, dt)
    ...
    -- Allocate entities by percentage
    local body_count = math.max(8, math.floor(n * 0.16))
    local head_count = math.max(4, math.floor(n * 0.08))
    local lwing_count = math.max(10, math.floor(n * 0.23))
    local rwing_count = math.max(10, math.floor(n * 0.23))
    local tail_count = math.max(8, math.floor(n * 0.20))
    local crest_count = math.max(4, math.floor(n * 0.06))
    local fire_count = n - body_count - head_count - lwing_count - rwing_count - tail_count - crest_count
    if fire_count < 0 then
        tail_count = math.max(4, tail_count + fire_count)
        fire_count = 0
    end
    ...
    local head_r = 0.025
    local head_sphere_count = math.max(3, head_count - 3)
