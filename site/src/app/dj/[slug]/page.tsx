  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center pt-20">
        <Spinner size="md" label="Loading DJ profile" />
      </div>
    );
  }
...
<div className="flat-card relative w-full max-w-md rounded-2xl p-10 text-center">
  ...
  return (
    ...
    <div className="min-h-screen pt-16">
      ...
      {/* Profile content */}
      <div className="relative mx-auto max-w-3xl px-6 -mt-16 pb-24">
        {/* Avatar */}
        <div className="mb-6">
          {profile.avatar_url ? (
            <Image
              ...
              width={112}
              height={112}
              className="h-28 w-28 rounded-full border-4 border-bg-primary object-cover"
              /* User-uploaded avatar URLs are dynamic (S3 presigned); cannot add to remotePatterns */
              unoptimized
            />
          ) : (
            <div
              className="flex h-28 w-28 items-center justify-center rounded-full border-4 border-bg-primary font-heading text-3xl font-bold text-white"
        ...
        <h1 className="mb-4 font-heading text-4xl font-bold leading-[1.05] sm:text-5xl">
          {profile.dj_name}
        </h1>
        ...
        {(profile.soundcloud_url || profile.spotify_url || profile.website_url) && (
          <div className="flat-card rounded-xl p-5">
            <h3 className="mb-3 font-mono text-[11px] font-semibold uppercase tracking-wider text-text-secondary">
              Links
            </h3>
            ...
            {profile.soundcloud_url && (
              <a
                ...
                className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/[0.03] px-4 py-2 text-sm text-text-secondary transition-colors hover:border-[#ff5500]/30 hover:text-[#ff5500]"
              >
            ...
            {profile.spotify_url && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/[0.03] px-4 py-2 text-sm text-text-secondary transition-colors hover:border-[#1DB954]/30 hover:text-[#1DB954]"
              >
                <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
            ...
            {profile.website_url && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/[0.03] px-4 py-2 text-sm text-text-secondary transition-colors hover:border-white/30 hover:text-white"
              >
              ...
              {BAND_LABELS.map((label, i) => (
                profile.block_palette![i] && (
                  ...
                  >
                    ...
                    {" "}
                    {profile.block_palette![i]!.replace(/_/g, " ").toLowerCase().replace(/\b\w/g, (c: string) => c.toUpperCase())}
                  </span>
                )
              ))}
            </div>
