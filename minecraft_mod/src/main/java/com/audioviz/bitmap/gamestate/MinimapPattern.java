 ...
 * to the stage center.
 *
 * <p>Ported from Paper: AudioVizPlugin → MinecraftServer,
 * Bukkit.getOnlinePlayers() → server.getPlayerManager().getPlayerList(),
 * Player.getLocation() → ServerPlayerEntity.getX()/getZ().
 */
public class MinimapPattern extends BitmapPattern {
    ...
    public MinimapPattern(MinecraftServer server) {
    ...
    private void refreshPlayers(int bufW, int bufH) {
        ...
        for (ServerPlayerEntity p : server.getPlayerManager().getPlayerList()) {
            ...
            double relX = (p.getX() - centerX) / (range * 2) + 0.5;
            double relZ = (p.getZ() - centerZ) / (range * 2) + 0.5;
            int px = (int) (relX * bufW);
            int py = (int) (relZ * bufH);
            dots.add(new PlayerDot(p.getName().getString(), px, py));
        }
    }
