...
class ShowStorage:
    ...
    def create_demo_show(self) -> Show:
        ...
        """
        from .models import Cue, CueAction, CueType, Transition, TransitionType, TriggerType

        show = Show(
            name="Demo Show",
            duration=60000,  # 1 minute
            bpm=128.0,
            metadata={"author": "AudioViz", "song_name": "Demo Track", "song_artist": "Test"},
        )

        # Add pattern cues
        pattern_track = show.get_track("patterns")
        if pattern_track:
            pattern_track.add_cue(
                Cue(
        ...
        if effect_track:
            ...
            effect_track.add_cue(
                Cue(
                    ...
                    duration=1000,
                    action=CueAction(
                        effect={"type": "bass_drop", "intensity": 1.0, "duration": 1000}
                    ),
                )
            )
