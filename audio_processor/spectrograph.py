...
# Band colors (matching the web UI)
BAND_COLORS = [
    Colors.BRIGHT_RED,      # Sub-bass
    Colors.YELLOW,          # Bass
...
class TerminalSpectrograph:
    ...
    def display(self, bands, amplitude=0.0, is_beat=False, beat_intensity=0.0):
        ...
        if self._preset == "auto":
            ...
            stats = f"{Colors.DIM}{bpm_str} {var_str} atk={self._attack:.2f} thr={self._beat_threshold:.2f}{Colors.RESET}"
        else:
            stats = f"{Colors.DIM}atk={self._attack:.2f} rel={self._release:.2f} thr={self._beat_threshold:.2f}{Colors.RESET}"
